...
export function ToolbarMenu<F extends string = string>({ selectMode, sort, filter, layout, extraItems, otherItems, bottomItem }: ToolbarMenuProps<F>) {
  ...
  return (
    <Menu title="" systemImage="ellipsis">
      ...
      {hasSelectMode ? (
        <>
          <Button title={selectMode.isSelectMode ? "完成选择" : "选择"} systemImage="checkmark.circle" action={selectMode.onToggleSelectMode} />
      ...
      {hasSort ? (
        <>
          ...
          {FILE_SORT_TOGGLES.map((toggle) => {
            ...
            const displayedTitle = toggle.togglable ? `${toggle.title} (${currentDir === "asc" ? "最旧" : "最新"})` : toggle.title;
            return (
              <Button
                key={toggle.key}
