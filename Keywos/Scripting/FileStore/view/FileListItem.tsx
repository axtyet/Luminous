...
function buildContextMenu(items?: ContextMenuItem[]) {
  ...
  return {
    menuItems: (
      <Group>
        {items.map((item, idx) => (
          <Button key={idx} title={item.title} systemImage={item.systemImage} role={item.role} action={item.action} />
        ))}
      </Group>
    ),
  };
...
const EXIF_KEY_MAP: Record<string, string> = {
  ...
  "exif.FocalLength": "焦距",
  "exif.FocalLenIn35mmFilm": "等效焦距（35mm）",
  "exif.LensModel": "镜头型号",
  "exif.LensMake": "镜头制造商",
  ...
  "exif.Saturation": "饱和度",
  "exif.Sharpness": "锐度",
  "exif.ImageDescription": "图像描述",
  "exif.UserComment": "用户备注",
  "exif.DigitalZoomRatio": "数字变焦比",
  "exif.FlashEnergy": "闪光灯能量",
  "exif.SubjectDistance": "主体距离",
  "exif.MaxApertureValue": "最大光圈",
  "exif.FileSource": "文件来源",
  "exif.SceneType": "场景类型",
  "exif.SensingMethod": "传感方式",
  "exif.CFAPattern": "CFA 图案",
  ...
  "gps.Altitude": "海拔",
  "gps.Speed": "速度",
  "gps.ImgDirection": "图像方向",
  "gps.HPositioningError": "水平定位误差",
  "gps.TimeStamp": "GPS时间戳",
  "gps.DateStamp": "GPS日期",
  ...
  "iptc.Byline": "作者",
  "iptc.City": "城市",
  "iptc.CountryPrimaryLocationName": "国家",
...
function formatExifValue(key: string, value: any): string {
  ...
  if (key === "exif.ExposureProgram")
    return ({ 0: "未定义", 1: "手动", 2: "程序自动", 3: "光圈优先", 4: "快门优先", 5: "创意", 6: "运动", 7: "人像", 8: "风景" } as Record<number, string>)[value] || "未知";
  ...
  if (key === "exif.SensitivityType")
    return ({ 0: "未知", 1: "SOS", 2: "REI", 3: "ISO Speed", 4: "SOS+REI", 5: "SOS+ISO", 6: "REI+ISO", 7: "SOS+REI+ISO" } as Record<number, string>)[value] || `未知(${value})`;
  ...
  if (key === "exif.SensingMethod")
    return (
      (
        { 1: "未定义", 2: "单芯片彩色面阵传感器", 3: "双芯片彩色面阵传感器", 4: "三芯片彩色面阵传感器", 5: "顺序彩色面阵传感器", 7: "三线扫描传感器", 8: "顺序彩色线扫描传感器" } as Record<
          number,
          string
        >
      )[value] || `未知(${value})`
...
function flattenMeta(meta: any, prefix = ""): Array<{ key: string; label: string; value: string }> {
  ...
  for (const k of Object.keys(meta)) {
    ...
    if (typeof val === "object" && !Array.isArray(val)) {
      result.push(...flattenMeta(val, fullKey));
      continue;
    }
...
export function FileInfoDialog({ file, nested }: { file: FileInfo; nested?: boolean }) {
        ...
        if (isLive) {
          ...
          if (data) {
            ...
            if (unpacked) {
              ...
              try {
                await FileManager.remove(imgTmp);
              } catch {}
              // 读取视频元数据
              ...
              try {
                await FileManager.remove(vidTmp);
              } catch {}
  ...
  const exifItems = imageMeta ? (imageMeta._livePhoto ? [] : flattenMeta(imageMeta)) : [];
      ...
      >
        ...
        <Section title="文件详情">
          ...
          <HStack spacing={12}>
            ...
            <Spacer />
            <Text font="body">
              {fmtSize(file.size)}（{file.size.toLocaleString()} 字节）
            </Text>
          </HStack>
          <HStack spacing={12}>
            <Text font="body" foregroundStyle="secondaryLabel">
