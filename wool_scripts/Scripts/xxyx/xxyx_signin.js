 ...
 * QX task：
 * 16 7 * * * https://raw.githubusercontent.com/fmz200/wool_scripts/main/Scripts/xxyx/xxyx_signin.js, tag=晓晓优选每日任务, img-url=https://raw.githubusercontent.com/fmz200/wool_scripts/main/icons/apps/xxyx.jpeg, enabled=true
 */
...
async function startTasks() {
  ...
  for (const item of json_token) {
    ...
    let energy = 0; // 总共获取的能量

    let taskList = await getTaskList(token);
    // console.log("收到响应：" + taskList);
        ...
        if (task.taskId === 2 || task.taskId === 3) {
          ...
          if (remainTasks > 0) {
            for (let i = 0; i < remainTasks; i++) {
              // 执行任务
              const success = await completeTask(token, task.taskId);
              if (success) {
                energy += task.energyBase;
              }
            }
          } else {
            console.log(`当前任务类型[${task.taskId}]已经完成`);
          }
        }
      }
      const subMsg = `[${username}]本次运行共获得${energy}能量`;
      if (!isNode) {
        $.msg($.name, subMsg, '', {'open-url': '', 'media-url': avatar});
      } else {
...
function singIn(token) {
  ...
  let options = {
    url: `https://xxyx-client-api.xiaoxiaoyouxuan.com/client/energy/mall/signIn`,
    headers: {
...
function getTaskList(token) {
  ...
  return $.http.get(options).then((resp) => {
    console.log("每日任务列表：" + resp.body);
    let rsp_body = JSON.parse(resp.body);
    if (rsp_body.success) {
      taskList = rsp_body.data;
