import { ViewPgcAny } from "@biliverse/protobuf/bilibili/app/viewunite/pgcanymodel.js";
import { ViewReply } from "@biliverse/protobuf/bilibili/app/viewunite/v1/viewunite.js";
import gRPC from "@nsnanocat/grpc";
import { URL } from "@nsnanocat/url";
import { Lodash as _, $app, Console, Storage } from "@nsnanocat/util";
...
function getEpisodes(modules = []) {
	...
	let epids = episodes.map(episode => {
		Console.debug(`episode.id: ${episode?.id}`);
		Console.debug(`episode: ${JSON.stringify(episode)}`);
		return episode?.id
	});
	*/
...
function detectLocales(infoGroup = { seasonTitle: undefined, seasonId: undefined, epId: undefined, mId: undefined, evaluate: undefined }) {
	...
	function detectSeasonTitle(seasonTitle) {
		...
		//Console.debug([...infoGroup.seasonTitle?.matchAll(/[(\uFF08]([^(\uFF08)\uFF09]+)[)\uFF09]/g)]?.pop());
		//Console.debug([...infoGroup.seasonTitle?.matchAll(/[(\uFF08]([^(\uFF08)\uFF09]+)[)\uFF09]/g)]?.pop()?.[1]);
	...
	function detectTraditional(seasonTitle, evaluate) {
		...
		function isTraditional(strings = [""]) {
			...
			//console.debug("isTraditional: " + isTraditional)
...
function setCache(infoGroup = { seasonTitle: undefined, seasonId: undefined, epId: undefined, mId: undefined, evaluate: undefined }, episodes = [], cache = {}) {
	...
	//Console.debug(`Set Cache`, `cache: ${JSON.stringify(cache)}`);
	return isSaved;
}
